...
const stats = [
  { value: 1900, suffix: "+", label: "FILES" },
  { value: 519, suffix: "K+", label: "LINES OF CODE" },
  { value: 53, suffix: "+", label: "TOOLS" },
  { value: 95, suffix: "+", label: "COMMANDS" },
];
...
const HeroSection = () => {
  return (
    <section className="min-h-screen flex flex-col items-center justify-center px-4 text-center relative">
      <h1 className="text-5xl sm:text-7xl lg:text-8xl font-serif font-bold tracking-tight mb-6">
        <span className="text-text-bright">Claude Code</span>
        <br />
        <span className="text-primary">Student2Dev</span>
      </h1>
      ...
      <div className="flex gap-3 mb-8 flex-wrap justify-center">
        ...
        >
          <span className="text-cat-blue">?</span> Ask DeepWiki
        </a>
        <a
        ...
        >
          <span className="font-bold">Y</span> Featured on Hacker News
        </a>
      </div>
      ...
      <p className="max-w-2xl text-lg mb-12">
        <span className="text-text-bright font-medium">
          The agent loop, 50+ tools, multi-agent orchestration, and unreleased features
        </span>
        <span className="text-muted-foreground">, mapped straight from the source.</span>
      </p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-16 mb-16">
        {stats.map((stat) => (
          <div key={stat.label} className="text-center">
            ...
            <div className="text-xs tracking-[0.2em] text-muted-foreground mt-2">{stat.label}</div>
          </div>
        ))}
      </div>
