        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-8 gap-6">
          {toolGroups.map((group) => (
            <div key={group.title} className="flex flex-col gap-3">
              <div>
                <div className={`text-[11px] uppercase tracking-[0.25em] font-mono ${group.accent}`}>
                  {group.title}
                </div>
              ...
              <div className="flex flex-col gap-2">
                {group.tools.map((tool) => {
                  ...
                  return (
                    <div
                      key={tool}
                      className={`flex items-center justify-between rounded-lg border px-3 py-2 text-xs font-mono bg-zinc-950/40 transition-colors ${
                        isLocked
                    ...
                    >
                      <span>{tool}</span>
                      {isLocked ? <span className="text-[10px]">lock</span> : null}
                    </div>
                  );
                })}
              </div>
